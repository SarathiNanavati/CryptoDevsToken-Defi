...
export default function Home() {
  ...
  const _addLiquidity = async () => {
    try {
      const addEtherInWei = utils.parseEther(addEther.toString());

      if (!addCDTokens.eq(zero) && !addEtherInWei.eq(zero)) {
        const signer = await getProviderOrSigner(true);
        setLoading(true);
        await addLiquidity(signer, addCDTokens, addEtherInWei);
        setLoading(false);
    ...
    if (liquidityTab) {
            ...
            {utils.parseEther(reservedCD.toString()).eq(zero) ? (
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    console.log(BigNumber.from(utils.parseEther(e.target.value || "0")).toString());
      ...
      return (
        <div>
          ...
          <div className={styles.inputDiv}>
            {/* Convert the BigNumber to string using the formatEther function from ethers.js */}
            {console.log(tokenToBeReceivedAfterSwap)}
            {ethSelected
